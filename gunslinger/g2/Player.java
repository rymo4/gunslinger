...
public class Player extends gunslinger.sim.Player {
	...
	// build the dynamic 2D table to record history
	...
	public void init(int nplayers, int[] friends, int enemies[]) {
		...
		if (friends.length >= (nplayers / PASSIVE_ACTIVE)
				|| enemies.length >= (nplayers / PASSIVE_ACTIVE))
			passive = true;
		else
			passive = false;
		...
		priorityList = new PriorityQueue<PriorityTuple>(10,
				new PriorityTupleComparator());
	...
	public int shoot(int[] prevRound, boolean[] alive) {
		try {
			...
			if (roundsWithNoDeaths == 3 && !passive) {
				...
				// if all players but one who are still alive are friends, shoot
				// that player
				// but don't shoot neutrals if there are too many enemies since
				// it will be a net gain for others
				if (live_players - live_friends - 1 == 1)
					for (int i = 0; i < alive.length; i++)
						if (alive[i] && this.id != i && isEnemy(i))
							return i;
						else if (alive[i] && this.id != i && !isFriend(i)
								&& enemies.length <= nplayers / 2)
							return i;
				...
				for (int i = 0; i < whoShotWhomCount.length; i++) {
					...
					if (alive[i] && isEnemy(i))
						for (int j = 0; j < whoShotWhomCount.length; j++) {
							if (alive[i]
									&& alive[j]
									&& (whoShotWhomCount[j][i] != 0 || whoShotWhomCount[i][j] != 0))
				...
				// shoot in the endgame only if the conditions above are
				// satisfied
				if (shotBeforeDead && live_friends >= ENDGAME_MIN_ALIVE_FRIENDS
						&& targetList.size() != 0) {
			...
			// add the columns of prediction matrix to get player most likely to
			// be shot at
			for (int i = 0; i < nplayers; i++) {
			...
			for (int player = 0; player < prevRound.length; player++) {
				...
				gaugeSeverity(player, prevRound[player], alive, shotAt, shotBy,
						prevRound, next_round_sum);
			}
			...
			if (passive)
				while (priorityList.size() != 0
						&& firstTuple.priority == priorityList.peek().priority
						&& (isEnemy(priorityList.peek().playerId) || Enemyus(
								priorityList.peek().playerId, record)))
				// while(priorityList.size()!=0 &&
				// firstTuple.priority==priorityList.peek().priority &&
				// (isEnemy(priorityList.peek().playerId) ||
				// Enemyus(priorityList.peek().playerId, record)))
				{
					if (priorityList.size() != 0) {
	...
	public void predictNextRound(double[][] next_round, List<List<Integer>> m,
			boolean[] alive) {
		...
		for (int i = 0; i < rows; i++) {
			// if someone shoot the other last round, most likely the other will
			// shot back next_round
	...
	private void gaugeSeverity(int shooter, int target, boolean[] alive,
			boolean[] shotAt, int[] shotBy, int[] prevRound,
			double[] next_round_sum) {
		...
		if (isEnemy(target) && (!shotAt[shooter] || !alive[shotBy[shooter]])
				&& alive[shooter] && alive[target])
			priorityList.add(new PriorityTuple(target, SEC_PRI));

		// shot by enemy when silent
		if (isEnemy(shooter) && prevRound[target] == -1 && alive[shooter]
				&& alive[target])
			priorityList.add(new PriorityTuple(shooter, SEC_PRI));

		// (similar to above case) when enemy shoots someone, whose target it
		// now dead
		if (isEnemy(shooter) && prevRound[shooter] != -1
				&& prevRound[prevRound[shooter]] != -1
				&& !alive[prevRound[prevRound[shooter]]] && alive[shooter]
				&& alive[prevRound[shooter]])
			priorityList.add(new PriorityTuple(shooter, SEC_PRI));

		// friend is shot by someone when he doesn't shoot
		if (isFriend(target) && prevRound[target] == -1 && alive[target]
				&& alive[shooter]) {
		...
		// e -> f
		if (isFriend(target) && isEnemy(shooter) && alive[target]
				&& alive[shooter]) {
		...
		// Lowest priority.
		// n -> e
		if (isNeutral(shooter) && isEnemy(target) && alive[target]
				&& alive[shooter] && next_round_sum[target] >= THRESHOLD) {
			priorityList.add(new PriorityTuple(target, NEUTRAL_CASES));
		}

		// If n -> f
		if (isNeutral(shooter) && isFriend(target) && alive[target]
				&& alive[shooter] && next_round_sum[target] >= THRESHOLD) {
			priorityList.add(new PriorityTuple(shooter, NEUTRAL_CASES));
		}

		// e -> n
		if (isNeutral(target) && isEnemy(shooter) && alive[shooter]
				&& alive[target] && next_round_sum[shooter] >= THRESHOLD) {
			priorityList.add(new PriorityTuple(shooter, NEUTRAL_CASES));
		}

		// If f -> n,
		if (isFriend(shooter) && isNeutral(target) && alive[target]
				&& alive[shooter] && next_round_sum[target] >= THRESHOLD) {
			priorityList.add(new PriorityTuple(target, NEUTRAL_CASES));
		}
